"""Cross-validated logistic-regression (linear probe) runner that logs per-run metrics and aggregates summaries."""
# imports
...
RUN_RESULTS_FILENAME = "linear_probe_run_results.csv"
...
def run_config(yaml_name: str, extraction_strategy: str, chip_gdf, logreg_max_iter: int = 1000):
    """Run stratified 5-fold CV linear probe for a given config/strategy, log metrics, and save per-run results.
    ...
    Args:
        ...
        logreg_max_iter: Max iterations for LogisticRegression solver.
    ...
    log_file = LOG_DIR / f"{Path(yaml_name).stem}_{extraction_strategy_lower}_linearprobe.log"
    logger.remove()
    ...
    run_results_path = output_dir / RUN_RESULTS_FILENAME
    if run_results_path.exists():
        logger.info(f"{run_results_path} exists, skipping")
        return
    ...
    for i, (train_idx, test_idx) in enumerate(tqdm(cv.split(X, labels), total=cv.get_n_splits(), desc=f"{yaml_name} folds")):
        # Fit scaler and probe on train fold, transform/train/predict on test fold
        scaler = StandardScaler()
        X_train = scaler.fit_transform(X[train_idx])
        X_test = scaler.transform(X[test_idx])
        clf = LogisticRegression(max_iter=logreg_max_iter, multi_class="auto", n_jobs=1)
...
def main(
    ...
    workers: Optional[int] = typer.Option(
        5, "--workers", "-w", help="Maximum number of parallel processses to deploy"
    )
):
    """
    ...
    logger.remove()
    logger.add(LOG_DIR / "linear_probe_main.log", level="INFO", enqueue=True)
    ...
    with ProcessPoolExecutor(max_workers=workers) as ex:
        ...
        futures = {
            ex.submit(run_config, yaml_name, extraction_strategy, chip_gdf, logreg_max_iter): (yaml_name, extraction_strategy)
    ...
    summary_root = PROCESSED_DATA_DIR / DATA_VERSION
    summary_root.mkdir(parents=True, exist_ok=True)
    summary_df = collect_run_results(summary_root)
    if summary_df.empty:
        logger.warning("No per-run results available for summary.")
    else:
        summary_csv_path = summary_root / "linear_probe_results_summary.csv"
        summary_df.to_csv(summary_csv_path, index=False)
        logger.info(f"Saved summary linear probe results to {summary_csv_path}")
