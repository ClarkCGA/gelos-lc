...
class PrithviViT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.temporal_encoding:
            ...
            self.temporal_embed_enc = TemporalEncoder(self.embed_dim, coords_scale_learn)
        if self.location_encoding:
            self.location_embed_enc = LocationEncoder(self.embed_dim, coords_scale_learn)

        self.cls_token = nn.Parameter(torch.zeros(1, 1, self.embed_dim))
        self.register_buffer("pos_embed", torch.zeros(1, self.patch_embed.num_patches + 1, self.embed_dim))

        # Transformer layers
        self.blocks = []
        for i in range(self.depth):
            self.blocks.append(Block(self.embed_dim, num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer))
            self.feature_info.append(
                {"num_chs": self.embed_dim * self.patch_embed.patch_size[0], "reduction": 1, "module": f"blocks.{i}"}
            )
