...
def generate(
    ...
    processed_data_dir: Path = typer.Option(Path("/app/data/processed"), help="Processed data directory"),
    interim_data_dir: Path = typer.Option(Path("/app/data/interim"), help="Interim data directory"),
    data_version: str = typer.Option("v0.50.1", help="Data version string"),
    configs_dir: Path = typer.Option("/app/configs", help="Directory containing experiment YAML configs"
    ),
):
    ...
    chip_gdf_chip_tracker.to_file(
        interim_dir / "gelos_app_chip_tracker.geojson", driver="GeoJSON"
    )
